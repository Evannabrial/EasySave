...
public class Differential : ITypeSave
{
        ...
        string encryptionKey = "")
    {
        ...
        {
            return new Full().StartSave(job, logType, pauseEvent, token, fileExtensions, listBlockProcess,
                enableEncryption,
        ...
        // --- 2. GESTION DU COMPTEUR DE PRIORITÉS ---
        int totalPrioFiles = 0;

        if (isDirectory)
        {
            // On compte les fichiers prioritaires QUI ONT ETE MODIFIES depuis la dernière save
            totalPrioFiles = DefinePriorityFilesDiff(fileExtensions, job, dateLast);
        }
        else
        {
            // Fichier unique
            if (fileExtensions != null &&
                fileExtensions.Contains(Path.GetExtension(job.Source), StringComparer.OrdinalIgnoreCase))
            {
                // Vérifier s'il doit vraiment être copié (différentiel)
        ...
        try
        {
                ...
                try
                {
                    ...
                    if (File.GetLastWriteTime(job.Source) > File.GetLastWriteTime(pathLastSave))
                    {
                        ...
                        if (limitBytes > 0 && fileSizeBytes > limitBytes)
                        {
                            ...
                            {
                                TransferManager.LargeFileSemaphore.Release();
                            }
                        }
                        else
                        {
                ...
                if (Directory.Exists(actual))
                {
                    foreach (string el in Directory.EnumerateFileSystemEntries(actual))
                    {
                        ...
                        if (Directory.Exists(el) && !marked.Contains(el))
                        {
                            ...
                            if (!Directory.Exists(pathLastSave + pathToCreate) &&
                                actualDirInfo.LastWriteTime > dateLast)
                            {
                                Stopwatch startTime = Stopwatch.StartNew();
                        ...
                        else
                        {
                            try
                            {
                                if (File.GetLastWriteTime(el) > dateLast)
                                {
                                    ...
                                    {
                                        // === MISE EN ATTENTE (Fichier Normal) ===
            ...
            foreach (string el in pendingNormalFiles)
            {
                ...
                try
                {
                    // Re-créer les dossiers au cas où
                    MatchCollection directories = Regex.Matches(pathToCreate,  @"[^\\]+(?=\\)");
                    string pathNewDirectory = "";
                    foreach (Match match in directories)
                    {
    ...
    private int DefinePriorityFilesDiff(List<string> fileExtensionsPrio, Job job, DateTime dateLast)
    {
        ...
        int myPendingPriorityFiles = 0;

        foreach (var file in allFiles)
        {
            // On ne compte que les fichiers qui vont EFFECTIVEMENT être copiés (modifiés depuis dateLast)
            if (File.GetLastWriteTime(file) > dateLast)
            {
                string ext = Path.GetExtension(file);
                if (fileExtensionsPrio.Contains(ext, StringComparer.OrdinalIgnoreCase))
                {
                    myPendingPriorityFiles++;
                }
            }
        }
